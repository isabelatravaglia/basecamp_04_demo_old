...
mod tests{
    ...
    fn test_01_constructor(){
        ...
        // Test decimals
        let res_decimal = ERC20::get_decimals();
        assert(res_decimal == decimals, 'Decimals do not match.');

        // Test total_supply
        let res_tot_supply = ERC20::get_total_supply();
        assert(res_tot_supply == initial_supply, 'Total supply does not match.');

        // Test the balance of the account variable
        let res_balance = ERC20::balance_of(account);
        assert(res_balance == u256_from_felt252(2000), 'Balance does not match.');
